...
router.post("/create", (req, res) => {
	...
	if (errors.length > 0) {
		...
		bcrypt.genSalt(10, (err, salt) => {
			bcrypt.hash(newVendorlog.password, salt, (err, hash) => {
				newVendorlog.password = hash;
				newVendorlog
					.save()
					.then(savedUser => {
						req.flash(
							"success_message",
							`User ${newVendorlog.username} was successfully created`
						);
...
router.get("/edit/:id", (req, res) => {
	Vendorlog.findOne({ _id: req.params.id }).then(vendorlogs => {
		Vendor.find({}).then(vendors => {
			res.render("admin/vendorlog/edit", {
